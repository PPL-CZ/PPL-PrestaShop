...
namespace PPLShipping\GuzzleHttp\Handler;

use PPLShipping\GuzzleHttp\Exception\ConnectException;
use PPLShipping\GuzzleHttp\Exception\RequestException;
use PPLShipping\GuzzleHttp\Promise as P;
use PPLShipping\GuzzleHttp\Promise\FulfilledPromise;
use PPLShipping\GuzzleHttp\Promise\PromiseInterface;
use PPLShipping\GuzzleHttp\Psr7\LazyOpenStream;
use PPLShipping\GuzzleHttp\TransferStats;
use PPLShipping\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \PPLShipping\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \PPLShipping\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \PPLShipping\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \PPLShipping\GuzzleHttp\Psr7\Utils::streamFor($sink);
