...
use PPLShipping\Jane\Component\OpenApi2\JaneOpenApi as OpenApi2Base;
use PPLShipping\Jane\Component\OpenApi3\JaneOpenApi as OpenApi3Base;
use PPLShipping\Jane\Component\OpenApiCommon\Exception\CouldNotParseException;
use PPLShipping\Jane\Component\OpenApiCommon\Exception\OpenApiVersionSupportException;
    ...
    private function isOpenApi2(string $origin) : array
    {
        ...
        if (\class_exists(OpenApi2Base::class)) {
            if (!\array_key_exists(OpenApi2Base::class, $this->schemaParsers)) {
                ...
                $this->schemaParsers[OpenApi2Base::class] = new \PPLShipping\Jane\Component\OpenApi2\SchemaParser\SchemaParser($openApi2Serializer);
            }
            try {
    ...
    private function isOpenApi3(string $origin) : array
    {
        ...
        if (\class_exists(OpenApi3Base::class)) {
            if (!\array_key_exists(OpenApi3Base::class, $this->schemaParsers)) {
                ...
                $this->schemaParsers[OpenApi3Base::class] = new \PPLShipping\Jane\Component\OpenApi3\SchemaParser\SchemaParser($openApi3Serializer);
            }
            try {
